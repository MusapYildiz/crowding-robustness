"""
data/openimages/dataset.py

Open Images icin PyTorch Dataset siniflari.
...
class IsolationDataset(Dataset):
    ...
    def _load_samples(self, split: str) -> list:
        ...
        for label in self.target_classes:
            d = self.dataset_dir / split / label
            if not d.exists():
                continue
    ...
    def get_class_counts(self) -> dict:
        counts = {c: 0 for c in self.target_classes}
...
class CrowdingDataset(Dataset):
    ...
    def _load_samples(self) -> list:
        samples = []
        for label in self.target_classes:
            label_dir = self.composite_dir / label
            if not label_dir.exists():
                continue
            for ft_dir in sorted(label_dir.iterdir()):
                ft = ft_dir.name
